...
export function EngagementExecutionWorkspace({
  ...
  const internalSequence: EngagementWorkspaceTab[] = ["overview", "tasks", "documents", "deliverables", "finance", "completion"];
  const currentInternalIndex = internalSequence.indexOf(activeTab);
  const laterIncompleteTab = internalSequence
    .slice(Math.max(0, currentInternalIndex + 1))
    .find((tab) => !sectionState[tab].complete);
  const firstOtherIncompleteTab = internalSequence.find((tab) => tab !== activeTab && !sectionState[tab].complete);
  const internalContinueTab = laterIncompleteTab
    ?? firstOtherIncompleteTab
    ?? internalSequence[Math.min(Math.max(0, currentInternalIndex + 1), internalSequence.length - 1)];
  const clientSequence: EngagementWorkspaceTab[] = ["overview", "documents", "deliverables", "messages", "finance", "timeline", "completion"];
  const clientCurrentIndex = Math.max(0, clientSequence.indexOf(activeTab));
  const requestedDocumentTab = outstandingActions.some((action) => Boolean(action.requiredDocumentType)) ? "documents" : null;
  const continueTab: EngagementWorkspaceTab = isClient
    ? requestedDocumentTab && requestedDocumentTab !== activeTab
      ? requestedDocumentTab
      : clientSequence[Math.min(clientCurrentIndex + 1, clientSequence.length - 1)]
    : internalContinueTab;
    ...
    <div className="grid w-full min-w-0 max-w-full gap-4 overflow-x-clip pb-24 md:pb-8">
      ...
      {activeTab === "overview" ? (
        <MobileSection title="Overview">
          ...
          <div className="mb-4"><EngagementStageProgress canManage={editable && (isAdmin || (isEngagementManager && assignedToEngagement))} returnPath={basePath} workflow={workflow} /></div>
      ...
      {activeTab === "completion" ? (
        <MobileSection title="Completion">
          ...
          <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <Card className="h-fit shadow-none"><CardHeader><CardTitle>{workflow.status === "active" ? "Complete or archive engagement" : workflow.status === "completed" ? "Archive engagement" : "Read-only record"}</CardTitle><CardDescription>{workflow.status === "active" ? "Complete the work when it is ready, or archive the engagement if the work is incomplete or no longer active." : workflow.status === "completed" ? "Create one ZIP package containing the workflow, files, messages, finance, timeline, approvals, and audit history." : "Archived engagements cannot be edited without administrator restoration."}</CardDescription></CardHeader><CardContent>{isAdmin && workflow.status === "active" ? <div className="grid gap-3"><form action={completeEngagementAction} className="grid gap-3"><input name="workflowId" type="hidden" value={workflow.id} /><input name="returnPath" type="hidden" value={basePath} /><div className="grid gap-2"><Label htmlFor="completion-notes">Completion notes</Label><Textarea id="completion-notes" minLength={10} name="notes" placeholder="Summarize the completed work, outcome, and final client handover..." required /></div><SubmitButton pendingText="Completing engagement..."><CheckCircle2 className="h-4 w-4" />Complete engagement</SubmitButton></form><form action={archiveCompletedEngagementAction} onSubmit={(event) => { if (!confirm('Are you sure you want to archive this engagement?')) event.preventDefault(); }}><input name="workflowId" type="hidden" value={workflow.id} /><input name="returnPath" type="hidden" value={basePath} /><SubmitButton pendingText="Creating ZIP archive..."><Archive className="h-4 w-4" />Archive incomplete work</SubmitButton></form></div> : isAdmin && workflow.status === "completed" ? <form action={archiveCompletedEngagementAction} onSubmit={(event) => { if (!confirm('Are you sure you want to archive this engagement?')) event.preventDefault(); }}><input name="workflowId" type="hidden" value={workflow.id} /><input name="returnPath" type="hidden" value={basePath} /><SubmitButton pendingText="Creating ZIP archive..."><Archive className="h-4 w-4" />Create ZIP and archive</SubmitButton></form> : <p className="text-sm text-muted-foreground">{workflow.status === "active" ? "Only an administrator can complete or archive the engagement." : "No further editing is available."}</p>}</CardContent></Card>
          </div>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-30 px-4 md:hidden">
        <Link className={`${buttonClassName({ className: "pointer-events-auto mx-auto flex w-full max-w-md justify-center" })} shadow-lg`} href={`${basePath}?tab=${continueTab}`}>Continue working<ArrowRight aria-hidden="true" className="h-4 w-4" /></Link>
      </div>
    </div>
  );
}
