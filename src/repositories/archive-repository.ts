...
export async function permanentlyDeleteArchive(input: {
  ...
  await connectToDatabase();
  assertAnyArchivePermission(input.actor, ["archive.permanent_delete"]);
  ...
  if (raw.legalHoldStatus && raw.legalHoldStatus !== "released") {
    throw new Error("This archive cannot be deleted while a legal hold is active.");
  }

  if (raw.archivePackageStorageKey) {
    try {
      const configuration = getR2Configuration();
      await getR2Client().send(new DeleteObjectCommand({
        Bucket: configuration.bucketName,
        Key: raw.archivePackageStorageKey,
      }));
    } catch {
      throw new Error("The archived ZIP could not be removed. The archive record was kept unchanged.");
    }
  }

  const deleted = await ArchiveRecordModel.deleteOne({ _id: id }).exec();
  if (deleted.deletedCount !== 1) {
    throw new Error("Archive record could not be deleted.");
  }

  await Promise.all([
    ArchiveDeletionRequestModel.deleteMany({ archiveRecordId: id }).exec(),
    ArchiveRestoreRequestModel.deleteMany({ archiveRecordId: id }).exec(),
    LegalHoldModel.deleteMany({ archiveRecordId: id }).exec(),
  ]);

  await writeAuditLog({
    actor: input.actor,
    action: "archive.permanently_deleted",
    resourceType: "ArchiveRecord",
    resourceId: input.archiveRecordId,
    reason: "Archive package and register record permanently deleted by an administrator.",
    previousValues: {
      archiveReference: raw.archiveReference,
      recordId: raw.recordId,
      recordType: raw.recordType,
    },
  });
