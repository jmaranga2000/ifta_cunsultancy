...
import { normalizeRecipientEmail } from "@/lib/client-recipient";
...
export async function sendPublicContactReceiptEmailToUser(details: PublicContactDetails) {
  const settings = await getPlatformSettings();
  const env = getServerEnv();
  const recipientEmail = normalizeRecipientEmail(details.email);
  const supportEmail = normalizeRecipientEmail(
    settings.portal.supportEmail || settings.company.email || env.GMAIL_SMTP_USER || "",
  );
  const senderEmail = normalizeRecipientEmail(env.GMAIL_SMTP_USER || "");

  // A public acknowledgement must go to the visitor's entered address, never to an IFTA inbox.
  if (!recipientEmail || recipientEmail === supportEmail || recipientEmail === senderEmail) {
    return {
      delivered: false,
      recipient: recipientEmail,
      reason: "A public contact acknowledgement cannot be delivered to the company mailbox.",
    };
  }
